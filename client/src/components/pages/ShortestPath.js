...
`}
            ...
            <Collapsible label="Dijkstra's Algorithm Information">
                        ...
                        <p>Dijkstra's algorithm allows us to find the <mark>shortest path</mark> between any two vertices of a graph.It differs from the minimum spanning tree because the shortest distance between two vertices <mark>might not include all the vertices</mark> of the graph.
                        </p>
            </Collapsible>
            <Collapsible label="Breadth First Search Algorithm Information">
                        <h2>Breadth First Search</h2>
                        <p> BFS is a <mark>traversing algorithm</mark> where you should start traversing from a selected node (source or starting node) and traverse the graph layerwise thus exploring the neighbour nodes (nodes which are directly connected to source node). You must then move towards the next-level neighbour nodes.
As the name BFS suggests, you are required to <mark>traverse the graph breadthwise</mark> as follows:
1.First move horizontally and visit all the nodes of the current layer2.Move to the next layer
            ...
            <Collapsible label="Depth First Search Algorithm Information">
                        ...
                        <p>The DFS algorithm is a <mark>recursive algorithm</mark> that uses the idea of backtracking. It involves exhaustive searches of all the nodes by going ahead, if possible, else by backtracking.
                        Here, the word backtrack means that when you are moving forward and there are no more nodes along the current path, you move backwards on the same path to find nodes to traverse. <mark>All the nodes will be visited on the current path</mark> till all the unvisited nodes have been traversed after which the next path will be selected. 
